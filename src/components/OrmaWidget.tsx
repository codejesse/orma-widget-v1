...
export const OrmaWidget: React.FC<Props> = ({
  ...
  const desktopPositionClass = !isMobile ? (position === "bottom-left" ? "bottom-6 left-6" : "bottom-6 right-6") : ""
  ...
  return (
    <>
      ...
      <div
        className={`fixed z-50 bg-white rounded-3xl shadow-2xl overflow-hidden backdrop-blur-sm border border-white/20 transition-all duration-300 hover:shadow-3xl ${desktopPositionClass} ${
          isMobile ? "" : "w-[380px]"
        }`}
        style={{
      ...
      >
        {step === "type" ? (
          <div className={`flex flex-col ${isMobile ? "max-h-[calc(100vh-32px)]" : ""}`}>
            {/* Header */}
            <div
            ...
            {/* Scrollable Content */}
            <div className={`flex-1 ${isMobile ? "overflow-y-auto" : ""}`} style={{ scrollbarWidth: "thin" }}>
