...
export const OrmaWidget: React.FC<Props> = ({
  ...
  // Position-based styling logic
  const getPositionStyles = () => {
    switch (position) {
      case "inline":
        return {
          containerClass: "relative w-full max-w-md mx-auto",
          widgetClass: "w-full",
          showBackdrop: false,
          isFixed: false,
        }
      ...
      case "bottom-left":
        return {
          containerClass: `fixed z-50 ${isMobile ? "inset-0 flex items-center justify-center p-4" : "bottom-6 left-6"}`,
          widgetClass: isMobile ? "w-full max-w-md max-h-[90vh]" : "w-[380px]",
      ...
      default:
        return {
          containerClass: `fixed z-50 ${isMobile ? "inset-0 flex items-center justify-center p-4" : "bottom-6 right-6"}`,
          widgetClass: isMobile ? "w-full max-w-md max-h-[90vh]" : "w-[380px]",
          showBackdrop: isMobile,
          isFixed: true,
        }
  ...
  const WidgetContent = () => (
    <div
      className={`${positionConfig.widgetClass} bg-white rounded-3xl shadow-2xl overflow-hidden backdrop-blur-sm border border-white/20 transition-all duration-300 hover:shadow-3xl`}
      style={{
        boxShadow:
          position === "inline"
    ...
    >
      {step === "type" ? (
        <div
          className={`flex flex-col ${positionConfig.isFixed && (isMobile || position === "modal") ? "max-h-[90vh]" : ""}`}
        >
          {/* Header */}
          <div
          ...
          >
            ...
            <div className="relative z-10 flex justify-between items-center">
              <div className="flex items-center gap-3 sm:gap-4 flex-1 min-w-0">
                ...
                <div>
                  <h2 className={`${isMobile ? "text-lg" : "text-xl"} font-bold truncate drop-shadow-sm`}>Acme Inc.</h2>
          ...
          {/* Scrollable Content */}
          <div
            className={`flex-1 ${positionConfig.isFixed && (isMobile || position === "modal") ? "overflow-y-auto" : ""}`}
          ...
          >
            ...
            <div className="relative z-10 flex justify-between items-center">
              <div className="flex items-center gap-3 sm:gap-4 flex-1 min-w-0">
                ...
                <div>
                  <h2 className={`${isMobile ? "text-lg" : "text-xl"} font-bold truncate drop-shadow-sm`}>Acme Inc.</h2>
                  <p className={`text-white/80 ${isMobile ? "text-xs" : "text-sm"} font-medium`}>Share your feedback</p>
                </div>
              </div>
  ...
  return (
    <>
      {/* Backdrop for modal and mobile floating positions */}
      {positionConfig.showBackdrop && (
        <div
          className="fixed inset-0 bg-black/20 backdrop-blur-sm z-40 transition-opacity duration-300"
          onClick={position !== "inline" ? onClose : undefined}
        />
      )}

      {/* Widget Container */}
      <div className={positionConfig.containerClass}>
        <WidgetContent />
      </div>
    </>
  )
